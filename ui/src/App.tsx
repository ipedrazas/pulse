    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <main className="mx-auto max-w-7xl px-6 py-6">
        ...
        <section className="mb-8">
          ...
          {nodes.error && (
            <p className="text-sm text-red-400">Error: {nodes.error}</p>
          )}
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {nodes.data.map((agent) => (
                <NodeCard
                  ...
                  onClick={() =>
                    setSelectedNode(selectedNode === agent.name ? null : agent.name)
                  }
                />
              ))}
            </div>
        ...
        <section>
          ...
          {containers.error && (
            <p className="text-sm text-red-400">Error: {containers.error}</p>
          )}
